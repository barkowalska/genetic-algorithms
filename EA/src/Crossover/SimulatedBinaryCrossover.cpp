...
std::vector<std::vector<double>> SimulatedBinaryCrossover::cross(
  ...
  for (size_t j = 0; j < size; ++j) {
    ...
    if (m_distribution(m_generator) <= m_Pc) {
      ...
      child1[j] = 0.5 * (parent1[j] + parent2[j]) +
                  0.5 * beta * (parent1[j] - parent2[j]);
      child2[j] = 0.5 * (parent1[j] + parent2[j]) +
                  0.5 * beta * (parent2[j] - parent1[j]);
    } else {
      child1[j] = parent1[j];
      child2[j] = parent2[j];
    }
