...
std::vector<size_t>
TournamentSelection::selection(const std::vector<double> &fitnessValue) {
  std::vector<size_t> mating_pool(fitnessValue.size());
  std::uniform_int_distribution<size_t> m_distribution(0.0,
                                                       fitnessValue.size() - 1);
  ...
  for (size_t j = 0; j < fitnessValue.size(); j++) {
    ...
    for (size_t k : chosen) {
      if (fitnessValue[k] > fitnessValue[best])
        best = k;
    }
    mating_pool[j] = best;
  }
