...
SGA::SGA(std::function<double(double)> fitnessFunction, float pc, float pm,
         int popsize, int maxgen, int max, int min) {
...
double SGA::binary_decoding(const std::vector<bool> &location) {
  ...
  double phenotype =
      static_cast<double>(sum) / static_cast<double>(pow(2, location.size()));
  phenotype = phenotype * roznica;
  phenotype += m_min;
  return phenotype;
}
void SGA::cross_over(std::vector<bool> &locationA,
                     std::vector<bool> &locationB) {
...
double SGA::uniform_random(double min, double max) {
  static std::random_device
      rd; // Urządzenie losowe (niezależne źródło entropii)
